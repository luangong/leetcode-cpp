 ...
 public:
  ...
  std::vector<int> twoSum(const std::vector<int>& nums, int target) {
    ...
    for (int i = 0; i < nums.size(); ++i) {
      vec[i] = std::make_pair(nums[i], i + 1);
    }
...
class SolutionTestFixture : public testing::TestWithParam<TestCase> {
 public:
  ...
  // To access the test parameter, call `TestWithParam<T>::GetParam()`.
  void SetUp() override {}
...
//   // Access the test parameter with the `TestWithParam<T>::GetParam()` method
//   const json& testCase = testing::TestWithParam<json>::GetParam();
//   EXPECT_EQ(
//       testCase.at("expected"),
//       solution.twoSum(testCase.at("nums"), testCase.at("target")));
// }

TEST_P(SolutionTestFixture, TestTwoSum2) {
  const json& testCase = testing::TestWithParam<json>::GetParam();
  EXPECT_EQ(
      testCase.at("expected"),
      solution.twoSum2(testCase.at("nums"), testCase.at("target")));
}
 ...
 *
 * TODO: Make the path to testcases.json generic instead of containing the problem slug.
